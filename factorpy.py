...
def setup_for_eval(rules,constants, in_factory_def=False):
	factories = []
	input_id = 0
...
for f_name, (f_constants,f_rules,f_imports)in zip(factory_defs.keys(),factory_defs.values()):
	...
	facts, storage, in_out_ports = setup_for_eval(rules,const,True)
	print("list of internal factories",facts)
	if DEBUG:
		print("previewing internal factory")
		utils.pretty_print(facts,storage)
...
global_facts,global_storage = setup_for_eval(global_rules,global_constants)
